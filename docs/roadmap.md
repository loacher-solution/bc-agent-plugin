...
- **`FileSystemWatcher` re-index** in `bc-symbol-lookup` — currently indexes once at startup; document now says restart-only. Implementing the watcher unblocks mid-session symbol downloads.
